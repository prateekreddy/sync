...
the Plane network — which the host reaches directly under the bridge driver. Two
overrides exist for what it cannot infer:
...
Neither is normally needed. The agent-facing URL is a separate question — nothing
in the stack can know your proxy's public hostname, so the summary prints
`<gateway-url>` for you to substitute; see *Point an agent at it* below.
